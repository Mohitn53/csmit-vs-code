...
const styles = `
  ...
  .ru-title span { color: var(--primary); }
  ...
  .ru-drop-title { font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 700; color: var(--foreground); margin-bottom: 6px; }
  .ru-drop-sub { font-size: 12px; color: var(--muted-foreground); letter-spacing: 0.03em; }
  .ru-file-pill {
    display: inline-flex; align-items: center; gap: 8px;
    background: rgba(99, 102, 241, 0.12); border: 1px solid rgba(99, 102, 241, 0.25);
    border-radius: 8px; padding: 8px 14px; font-size: 12px; color: var(--foreground); margin-top: 14px;
  }
  .ru-file-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--primary); flex-shrink: 0; }

  /* Buttons */
  .ru-btn {
    width: 100%; padding: 15px 24px; background: var(--primary); color: #fff; border: none;
  ...
  .ru-btn:hover:not(:disabled) { background: #4f46e5; transform: translateY(-1px); box-shadow: 0 8px 24px rgba(99, 102, 241, 0.35); }
  ...
  /* Panel */
  .ru-panel { background: var(--card); border: 1px solid var(--border); border-radius: 16px; overflow: hidden; }
  .ru-panel-header {
    padding: 16px 20px; border-bottom: 1px solid var(--border);
    display: flex; align-items: center; gap: 10px;
  }
  .ru-panel-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--primary); }
  .ru-panel-dot.green { background: var(--accent); }
  .ru-panel-dot.amber { background: #fbbf24; }
  .ru-panel-dot.red   { background: #f87171; }
  .ru-panel-title {
    font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700;
    color: var(--foreground); text-transform: uppercase; letter-spacing: 0.08em;
  }
  ...
  .ru-panel-body::-webkit-scrollbar-thumb { background: var(--border); border-radius: 4px; }

  /* Parsed text */
  .ru-pre { font-family: 'DM Mono', monospace; font-size: 12px; line-height: 1.8; color: var(--muted-foreground); white-space: pre-wrap; word-break: break-word; }

  /* Original skill item (resume parser) */
  .ru-skill { display: flex; align-items: center; justify-content: space-between; padding: 9px 0; border-bottom: 1px solid var(--border); }
  .ru-skill:last-child { border-bottom: none; }
  .ru-skill-name { font-size: 12.5px; color: var(--foreground); font-weight: 500; }
  .ru-skill-bar-wrap { display: flex; align-items: center; gap: 8px; }
  .ru-skill-bar { width: 70px; height: 3px; background: var(--border); border-radius: 4px; overflow: hidden; }
  .ru-skill-fill { height: 100%; background: linear-gradient(90deg, var(--primary), #818cf8); border-radius: 4px; transition: width 0.6s ease; }
  .ru-skill-score { font-size: 11px; color: var(--primary); font-weight: 500; min-width: 32px; text-align: right; }

  /* Section */
  .ru-section-block { margin-bottom: 16px; }
  .ru-section-label { font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 700; color: var(--primary); text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 8px; }
  .ru-section-item { font-size: 12px; color: var(--muted-foreground); line-height: 1.6; padding: 5px 0 5px 12px; border-left: 2px solid var(--border); margin-bottom: 4px; }

  /* Empty state */
  .ru-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 48px 20px; gap: 12px; color: var(--muted-foreground); font-size: 12px; text-align: center; }
  ...
  .sa-section-title {
    ...
    color: var(--foreground); letter-spacing: -0.5px; margin-bottom: 4px;
  }
  .sa-section-title span { color: var(--primary); }
  .sa-section-sub { font-size: 11.5px; color: var(--muted-foreground); }
  ...
  .sa-score-track { fill: none; stroke: var(--border); stroke-width: 8; }
  .sa-score-fill { fill: none; stroke: var(--primary); stroke-width: 8; stroke-linecap: round; transition: stroke-dashoffset 1s ease; }
  .sa-score-text { position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .sa-score-num { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; color: var(--foreground); line-height: 1; }
  .sa-score-lbl { font-size: 9px; color: var(--primary); letter-spacing: 0.05em; }
  .sa-banner-meta { flex: 1; }
  .sa-profile-type { font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 800; color: var(--foreground); margin-bottom: 6px; }
  .sa-banner-stats { display: flex; gap: 18px; flex-wrap: wrap; }
  .sa-stat { font-size: 11.5px; color: var(--muted-foreground); }
  .sa-stat span { color: var(--foreground); font-weight: 500; }
  ...
  .sa-panel { background: var(--card); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; }
  .sa-panel-head { padding: 13px 17px; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 8px; }
  .sa-panel-title { font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 700; color: var(--foreground); text-transform: uppercase; letter-spacing: 0.08em; }
  .sa-panel-body { padding: 16px; max-height: 340px; overflow-y: auto; }
  .sa-panel-body::-webkit-scrollbar { width: 3px; }
  .sa-panel-body::-webkit-scrollbar-thumb { background: var(--border); border-radius: 3px; }

  /* Demand row */
  .sa-demand-row { display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border); }
  .sa-demand-row:last-child { border: none; }
  .sa-demand-left { display: flex; flex-direction: column; gap: 2px; }
  .sa-demand-skill { font-size: 12px; color: var(--foreground); font-weight: 500; }
  .sa-demand-why { font-size: 10px; color: var(--muted-foreground); line-height: 1.4; max-width: 200px; }
  .sa-demand-right { display: flex; align-items: center; gap: 7px; flex-shrink: 0; }
  .sa-demand-bar-wrap { width: 64px; height: 3px; background: var(--border); border-radius: 4px; overflow: hidden; }
  .sa-demand-fill { height: 100%; border-radius: 4px; background: linear-gradient(90deg, var(--primary), #818cf8); }
  .sa-demand-score { font-size: 10px; color: var(--primary); min-width: 24px; text-align: right; }
  ...
  /* Category coverage */
  .sa-cat-row { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid var(--border); }
  .sa-cat-row:last-child { border: none; }
  .sa-cat-label { font-size: 11px; color: var(--muted-foreground); min-width: 140px; }
  .sa-cat-bar-wrap { flex: 1; height: 4px; background: var(--border); border-radius: 4px; overflow: hidden; }
  .sa-cat-bar-fill { height: 100%; border-radius: 4px; background: linear-gradient(90deg, var(--primary), #818cf8); transition: width 0.8s ease; }
  .sa-cat-pct { font-size: 10px; color: var(--primary); min-width: 32px; text-align: right; }

  /* Gaps */
  .sa-gap-block { margin-bottom: 14px; }
  .sa-gap-block:last-child { margin-bottom: 0; }
  .sa-gap-cat-label { font-size: 10px; color: var(--primary); text-transform: uppercase; letter-spacing: 0.09em; margin-bottom: 6px; }

  /* Recommendations */
  .sa-rec { display: flex; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--border); align-items: flex-start; }
  ...
  .sa-rec-skill { font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 700; color: var(--foreground); margin-bottom: 2px; }
  .sa-rec-cat { font-size: 10px; color: var(--primary); margin-bottom: 3px; }
  .sa-rec-why { font-size: 11px; color: var(--muted-foreground); line-height: 1.5; }
  .sa-rec-badge {
  ...
  .ai-report-title {
    ...
    color: var(--foreground); letter-spacing: -0.5px;
  }
  .ai-report-title span { color: var(--accent); }
  .ai-report-badge {
  ...
  .ai-report-panel-head {
    padding: 14px 20px; border-bottom: 1px solid rgba(16, 185, 129, 0.1);
    display: flex; align-items: center; gap: 10px;
    background: rgba(16, 185, 129, 0.04);
  }
  .ai-report-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--accent); animation: pulse-green 2s infinite; }
  ...
  .ai-report-panel-title {
    ...
    color: var(--accent); text-transform: uppercase; letter-spacing: 0.09em;
  }
  .ai-report-panel-sub { font-size: 10.5px; color: var(--accent); margin-left: auto; }
  ...
  .ai-report-body::-webkit-scrollbar-thumb { background: var(--border); border-radius: 3px; }

  /* Markdown-like rendering for AI report */
  .ai-report-text { font-family: 'DM Mono', monospace; font-size: 12.5px; line-height: 1.9; color: var(--muted-foreground); }
  .ai-report-text .ai-h1 {
    font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 800;
    color: var(--foreground); margin: 24px 0 10px; letter-spacing: -0.3px;
    padding-bottom: 8px; border-bottom: 1px solid var(--border);
  }
  .ai-report-text .ai-h1:first-child { margin-top: 0; }
  .ai-report-text .ai-h2 {
    font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700;
    color: var(--accent); margin: 18px 0 8px; text-transform: uppercase; letter-spacing: 0.06em;
  }
  .ai-report-text .ai-bold { color: var(--foreground); font-weight: 500; }
  .ai-report-text .ai-p { margin-bottom: 10px; }
  .ai-report-text .ai-li {
    padding: 4px 0 4px 16px; border-left: 2px solid var(--border);
    margin-bottom: 6px; color: var(--muted-foreground);
  }
  ...
  .ai-model-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--accent); }
`;
...
function SkillAnalyzer({ skills }: { skills: string[] }) {
          ...
          <>
            ...
            <div className="sa-grid">
              ...
              <div className="sa-panel">
                ...
                <div className="sa-panel-body">
                  {Object.entries(category_breakdown).map(([cat, data]: [string, any]) => (
              ...
              <div className="sa-panel">
                ...
                <div className="sa-panel-body">
                  {Object.keys(gaps).length === 0
                    ...
                    : Object.entries(gaps).map(([cat, data]: [string, any]) => (
            ...
            <div className="ai-report-section">
              ...
              >
                {aiLoading
                  ? <><div className="ru-spinner" style={{ borderTopColor: 'var(--accent)' }} />Generating Career Report...</>
...
export default function ResumeAnalyzerPage() {
  ...
  const renderSkills = () => {
    ...
    const entries = Object.entries(analysis.skills).sort((a: any, b: any) => b[1] - a[1]);
    if (!entries.length) return <EmptyState icon="⚡" text="No skills detected" />;
    const max = entries[0][1] as number;
    return entries.map(([skill, score]: [string, any]) => (
  ...
  const renderSections = () => {
    ...
    return keys.map((k) => (
      <div className="ru-section-block" key={k}>
        ...
        {analysis.sections[k].map((s: string, i: number) => (
