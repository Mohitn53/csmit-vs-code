...
function Progress({ value, className, indicatorColor }: { value: number; className?: string; indicatorColor?: string }) {
  return (
    <div className={`w-full rounded-full bg-white/10 overflow-hidden ${className}`}>
      <div
        className={`h-full rounded-full transition-all duration-500 ${indicatorColor || 'bg-indigo-500'}`}
  ...
  useEffect(() => {
    async function fetchTopics() {
      ...
      if (data && !error) {
        ...
        const start = (parseInt(categoryId) - 1) * 20
        setTopics(allTopics.slice(start, start + 20))
      }
      setLoading(false)
    }
    fetchTopics()
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {topics.map((topic, i) => (
            <Link key={topic.id} to={`/dashboard/topic/${encodeURIComponent(topic.name)}`}>
              <motion.div
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="font-semibold text-lg group-hover:text-indigo-400 transition-colors line-clamp-1">
                    {topic.name}
                  </h3>
                  <div className={`flex items-center gap-1 text-sm font-medium ${topic.trend === 'up' ? 'text-green-400' : 'text-red-400'}`}>
                    {topic.score}
